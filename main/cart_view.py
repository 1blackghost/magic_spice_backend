...
def get_cart(request):
    try:
        ...
        cart_data = [{"cart_id":item.uid,'item': item.item, 'quantity': item.quantity, 'price': item.price,"img":item.img,"number":item.number,"tax":item.tax,"delivery_fees":item.delivery_fees,"other_fees":item.delivery_fees,"discount":item.discount,"product_id":item.product_id} for item in cart_items]
